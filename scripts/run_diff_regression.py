...
def tracker(arg):
    ...
    bash("cd %s && ln -sf ../nemu.out nemu.out" % running_directory)
    ...
    nemu_output = open("%s/nemu.out" % running_directory, "r")
    while True:
        emu_line = emu_output.readline()
        nemu_line = nemu_output.readline()
        if emu_line != nemu_line:
            status = "failure"
            break
        # reached end of file
        if emu_line == nemu_line and emu_line == "":
            status = "success"
            break
...
Args:
    ...
    with open("../%s_diff_regression_report" % test_name, "w") as f:
        f.write("%s" % result)
    # print out summary
    nsuccess = 0
    nfailure = 0
